...
class RenewBookInstancesViewTest(TestCase):
    ...
    def test_redirects_to_all_borrowed_book_list_on_success(self):
        login = self.client.login(username='testuser2', password='2HJ1vRV0Z&3iD')
        self.assertTrue(login)  # Ensure login is successful
        ...
        # Ensure that the response is a redirect to the borrowed books list
        self.assertRedirects(response, reverse('borrowed-books'))
        
        # Check if the test user can access the borrowed books list directly
        borrowed_books_response = self.client.get(reverse('borrowed-books'))
        
        self.assertEqual(borrowed_books_response.status_code, 200)
...
class AuthorCreateViewTest(TestCase):
    ...
    def setUp(self):
        # Create a user with permission to add authors
    ...
    def test_redirects_on_success(self):
        """Test that the view redirects to the author detail on success."""
        self.client.login(username='test_user', password='some_password')
        ...
        # Extract the primary key of the new author from the response
        new_author = Author.objects.get(first_name='Jane', last_name='Doe')
        expected_redirect_url = reverse('author-detail', args=[new_author.pk])
        
        # Check if the response redirects to the expected URL
        self.assertRedirects(response, expected_redirect_url)

    def test_initial_date_of_death(self):
        """Test that the initial date of death is set correctly."""
        self.client.login(username='test_user', password='some_password')
        response = self.client.get(self.url)
    ...
    def test_form_invalid_data(self):
        """Test that the form displays errors with invalid data."""
        self.client.login(username='test_user', password='some_password')
        response = self.client.post(self.url, {
            'first_name': '',
            'last_name': '',
