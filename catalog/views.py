...
class BorrowedBooksByUserLibrarianListView(PermissionRequiredMixin, generic.ListView):
    ...
    paginate_by = 10
    permission_required = ('catalog.can_mark_returned')  # change_book => Builtin permission
...
class AuthorDelete(PermissionRequiredMixin, DeleteView):
    model = Author
    success_url = reverse_lazy('authors')           # here we providing a reversed URL as the url attribute of a generic class-based view. so we use reverse_lazy()
...
class BookDelete(PermissionRequiredMixin,DeleteView):
    model = Book
    success_url = reverse_lazy('books')           # here we providing a reversed URL as the url attribute of a generic class-based view. so we use reverse_lazy()
